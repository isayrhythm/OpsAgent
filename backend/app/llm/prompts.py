ROUTER_SYSTEM_PROMPT = (
    ...
    "概念问答、对粘贴文本的解释/总结/改写都必须返回空 skill_names。"
    "不要因为文本里偶然出现某个领域词、ID 形态、英文缩写或历史中出现过 skill 相关内容就选择 skill；"
    "只有当前用户明确要求查询本地数据、执行计算或调用专门能力时才选择。"
    "如果不需要 skill，skill_names 必须是 []。resolved_message 只能补全历史中有依据的信息，不能添加新事实。"
    "只输出 JSON。"
)
